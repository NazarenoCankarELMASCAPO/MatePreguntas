if (parseInt(localStorage.getItem("points")) >= parseInt(localStorage.getItem("minimumScore"))) {
  ...
  Swal.fire({
    ...
    if (x) {
      playAudio("../genio.mp3");
      setTimeout(() => {
        document.querySelector("img").classList.add("image-animation");
        let audio = new Audio();
        audio.src = "../win.mp3";
        audio.loop = true;
        audio.play();
...
} else if (parseInt(localStorage.getItem("points")) < parseInt(localStorage.getItem("minimumScore"))) {
